...
type OpenAPIPreview = { endpoints: Array<{ method: string; path: string; status_code: number; body: string; content_type: string; scenarios?: Array<{ name: string; is_default: boolean; status_code: number; body: string; content_type: string; delay_ms: number; headers?: Record<string, string> }> }>; warnings?: string[] };
...
export default function Home() {
  ...
  async function previewOpenAPI(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setMessage(''); setOpenapiPreview(null);
    const response = await fetch(`${apiOrigin}/api/v1/import/openapi/preview`, { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: openapiDocument });
    if (!response.ok) { setError(await response.text()); return; }
    setOpenapiPreview(await response.json());
  }

  async function importOpenAPI() {
    if (!openapiPreview) return; setError(''); setMessage('');
    const response = await fetch(`${apiOrigin}/api/v1/mocks/import`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'OpenAPI import', endpoints: openapiPreview.endpoints }) });
    if (!response.ok) { setError(await response.text()); return; }
    const value = await response.json(); const firstPath = openapiPreview.endpoints[0].path;
    setResult({ key: value.public_key, url: `${apiOrigin}/m/${value.public_key}${firstPath}`, token: value.management_token, expires: value.expires_at });
    setMessage('Imported mock created. Review the generated routes before using it.');
  }
  ...
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} />
    <header className="site-header"><div className="topbar-left"><a className="brand" href="https://mikrolyt.com"><span className="brand-mark">M</span><span className="wordmark">mikrolyt<span className="dot">.</span></span></a><span className="product-tag">Mocklet · M02</span></div><nav aria-label="Main navigation"><a href="#how-it-works">How it works</a><a href="#case-study">Case study</a><a href="#playground">Try it</a></nav></header><div className="route-stripe" />
  ...
  "currency": "IDR"
    ...
    <section className="section" id="openapi-import"><div className="section-heading compact"><div><span className="eyebrow">Contract import</span><h2>Preview an OpenAPI contract.</h2></div><p>Paste an OpenAPI 3.x YAML or JSON document. Review the generated routes and scenarios before creating a disposable mock.</p></div><form onSubmit={previewOpenAPI}><label htmlFor="openapi-document">OpenAPI document</label><textarea id="openapi-document" value={openapiDocument} onChange={event => setOpenapiDocument(event.target.value)} placeholder="openapi: 3.0.3\ninfo:\n  title: Checkout\n  version: 1.0.0\npaths: {}" required /><button className="button secondary" type="submit">Preview contract <span>→</span></button></form>{openapiPreview && <section className="result"><strong>{openapiPreview.endpoints.length} route(s) generated for review</strong><pre><code>{JSON.stringify(openapiPreview.endpoints, null, 2)}</code></pre><p>Review the generated response examples and status scenarios above before activation.</p><button className="button primary" type="button" onClick={importOpenAPI}>Create reviewed mock <span>→</span></button></section>}</section>
    ...
    <footer><div className="brand"><span className="brand-mark">M</span><span className="wordmark">Mocklet</span></div><p>Build against the contract. Ship with confidence.</p><span className="by-mikrolyt">Part of the <a href="https://mikrolyt.com">Mikrolyt</a> developer tools.</span></footer>
  </main>;
}
