...
- Brand alignment (branch feature/brand-token-alignment, not merged): web palette re-mapped to Mikrolyt tokens (green #16A34A, yellow #F5C518, ink #111827, slate #1F2937, gray #647488; tints via color-mix); Space Grotesk headings + Inter body via next/font; header byline + footer attribution link to mikrolyt.com; metadata title/description say "by Mikrolyt" and og:site_name is Mikrolyt; next.config now sends global security headers (nosniff, X-Frame-Options SAMEORIGIN, Referrer-Policy strict-origin-when-cross-origin, Permissions-Policy, HSTS includeSubDomains). Verified with a local next build; browser pass pending.
