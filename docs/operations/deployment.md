...
Current state: deployed to `mocklet.mikrolyt.com` on `root@192.129.240.188`. Caddy routes `/api/*` and `/m/*` to the private API and the web root to the private web container. Immutable image, HTTPS, API/runtime smoke, existing-site regression, no-public-API-or-DB-port, encrypted off-host delivery, checksum, and scheduled-backup checks pass. The isolated decrypt/restore rehearsal remains pending because the private restore key is held on the homelab and is not available to the production VPS account; do not represent disaster-recovery readiness as complete until that rehearsal is recorded.
